...
def create_parser() -> ArgumentParser:
    ...
    rename = subparsers.add_parser(
        "rename-module", help="Rename a module. Note: could be either file or directory"
    )
    ...
    file_args.append(move_symbols.add_argument("source", help="Source filename"))
    move_symbols.add_argument("symbol", help="Symbols to move")
    file_args.append(move_symbols.add_argument("dest", help="Destination filename"))
...
def parse_config(parser: ArgumentParser) -> RefactorConfig:
    ...
    return RefactorConfig(
        action=action, config=config, is_dry_run=args.dry_run, is_verbose=args.verbose
    )
